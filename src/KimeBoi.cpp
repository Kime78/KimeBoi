...
void Processor::emulateCycle(std::string &output)
{
	...
	switch (Memory.read(pc))
	{
		...
		case 0xCE:
		{
			...
			Flags.H = (((Registers.A & 0xf) + (tmp & 0xf) + Flags.C) & 0x10) == 0x10;
			Flags.C = (((Registers.A & 0xff) + (tmp & 0xff)  + Flags.C) & 0x100) == 0x100;
			Flags.Z = (res == 0);
			Flags.N = 0;

			Registers.A += tmp + Flags.C;    
			pc += 2;
			break;					
		}
		case 0xD0:
