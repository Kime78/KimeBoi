...
void PPU::tick_ppu(int cycles, Processor::_Memory &mem,
  ...
  while (cycles >= 4) {
    ...
    case Vblank: {
      ...
      if (ppu_cycles == 0) {
        ...
        if ((mem.read(0xFF41, 0) & 0x10) >>
            4) // is stat vblank interrupt enabled
  ...
  if (ppu_line != 144) {
    for (int x = 0; x < 160; x++) {
      if (isBitSet(mem.read(0xff40, 0), 7)) {
        ...
        tile_y = ((y + SY) % 256) % 8;
        tile_index = mem.read(
            tilemap + ((x + SX) % 256) / 8 + (((y + SY) % 256) / 8) * 32, 0);
      ...
      if (sprites[i].y - 16 <= y && y < sprites[i].y + sprite_height - 16)
        if (spritenr < 10) {
          if (isBitSet(mem.read(0xff40, 0), 1)) {
            ...
            std::uint8_t palette =
                palette_id ? mem.read(0xFF49, 0) : mem.read(0xFF48, 0);
            std::uint8_t color0 = pixel_array[x][y]; // this is "transparent"
            std::uint8_t color1 =
                isBitSet(palette, 3) << 1 | isBitSet(palette, 2);
            std::uint8_t color2 =
                isBitSet(palette, 5) << 1 | isBitSet(palette, 4);
            std::uint8_t color3 =
                isBitSet(palette, 7) << 1 | isBitSet(palette, 6);
            ...
            if (isBitSet(sprites[i].attributes, 6)) {
              sprite_data1 =
                  mem.read(sprite_tile_index + 2 * (7 - sprite_tile_y), 0);
              sprite_data2 =
                  mem.read(sprite_tile_index + 2 * (7 - sprite_tile_y) + 1, 0);
            } else {
              sprite_data1 =
                  mem.read(sprite_tile_index + 2 * (sprite_tile_y), 0);
              sprite_data2 =
                  mem.read(sprite_tile_index + 2 * (sprite_tile_y) + 1, 0);
            }
...
void PPU::draw_window(Processor::_Memory &mem, uint8_t x, uint8_t y) {
  ...
  if (x >= WX && WLY >= WY && window_enabled) {
    ...
    tile_index = mem.read(w_tilemap + (x - WX) / 8 + ((WLY - WY) / 8) * 32,
                          0); // maybe no WX and WY
    if (tiledata == 0x8800) {
      if (tile_index < 128)
        tile_start = 0x9000 + tile_index * 16;
      else {
